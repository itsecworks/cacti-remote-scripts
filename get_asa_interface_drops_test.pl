...
if(!$myfirewallip||!$username||!$password||!$ifname){
	...
	print "Usage: query_asa_interface_drops.pl firewallip username password interfacename \n";
	exit;
}
...
foreach my $line (split /[\r\n]+/, $output) {
	...
	elsif ($line =~ /^\s+(\d+)\sinput\sreset\sdrops,\s(\d+)\soutput\sreset\sdrops$/m) {
		$input_rdrop		= $1;
		$output_rdrop		= $2;
		if ($interface_found eq $ifname) {
			print "Interface Name: $ifname\n";
			print "Input\n";
			print "input_packet: $input_packet\n";
			print "input_byte: $input_byte\n";
			print "input_nobuffer: $input_nobuffer\n";
			print "received_broadcast: $received_broadcast\n";
			print "received_runt: $received_runt\n";
			print "received_giant: $received_giant\n";
			print "input_error: $input_error\n";
			print "input_crc: $input_crc\n";
			print "input_frame: $input_frame\n";
			print "input_overrun: $input_overrun\n";
			print "Output\n";
			print "ouput_packet: $output_packet\n";
			print "output_byte: $output_byte\n";
			print "output_underrun: $output_underrun\n";
			print "output_pause: $input_pause\n";
			print "output_resume: $input_resume\n";
			print "output_error: $output_error\n";
			print "output_collision: $output_collision\n";
			print "output_interface_reset: $output_ifreset\n";
			print "ouput_late_collition: $output_lcollision\n";
			print "ouput_deferred: $output_deferred\n";
			print "input / output reset drops\n";
			print "input_rdrop: $input_rdrop\n";
			print "output_rdrop: $output_rdrop\n";
			$interface_found ='';
		}
	}
}